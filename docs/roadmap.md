...
- **Identity tables (Phase 2, built):** `identity/resolve.py` full-rebuilds `dim_ca`, `dim_ca_address`, `ca_amplemarket_user`, `company_crosswalk`, `contact_crosswalk`, `amplemarket_contact_map`, `identity_unresolved` from the raw layer — run manually until wired into the cron (see Phase 2 status).
