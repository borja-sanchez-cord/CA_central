...
- **Tables already in Supabase (raw landing layer):** `raw_amplemarket_tasks`, `raw_amplemarket_calls`, `raw_amplemarket_users` (the /users mirror — load-bearing for call attribution), `raw_hubspot_emails`, `raw_hubspot_meetings`, the roster mirrors `raw_hubspot_owners` (all owners + teams) and `config_ca_teams` (the CA-team policy, seeded from `config/ca_teams.json`), plus an `ingestion_runs` audit log. Each activity table has a few extracted columns + the full source payload in a `raw` jsonb column. Primary key = source id; re-runs are idempotent (`ON CONFLICT DO NOTHING`; meetings update-on-conflict). The **normalized** schema (activity fact + dimensions) does **not** exist yet — it begins at Phase 3.
- **Identity tables (Phase 2, built):** `identity/resolve.py` full-rebuilds `dim_ca`, `dim_ca_address`, `ca_amplemarket_user`, `company_crosswalk`, `contact_crosswalk`, `amplemarket_contact_map`, `identity_unresolved` from the raw layer — now wired into the daily cron (Phase 3).
- **Model layer (Phase 3, built):** `model/build_activity.py` full-rebuilds the **`activity` fact table** (one row = one real activity event, deduplicated + attributed; non-countable rows kept with `counts=false` + reason) from raw + identity tables; all judgment is in `model/rules.py` (pure functions, tested in `tests/` against real validated fixtures). Schema lives in Git migrations (`migrations/`, applied by `migrations/apply.py` in the daily run). **`activity_flat` is THE view downstream phases read** — never raw.
...
## Phase 2 — Identity resolution + account de-duplication  ✅ DONE (built `identity/resolve.py`; PM-validated + production-audited 2026-07-15)
...
- **People:** match by exact lowercased email.
- **Companies:** **collapse HubSpot duplicate companies (mandatory)** — merge only on domain match AND name-token agreement, never domain alone; guard free-email domains. (No fuzzy matching — the fuzzy-fallback plan was superseded 2026-07-15; see spec §9.)
- Materialize the resolved identities into queryable tables — as built: deterministic full-rebuild each run, no incremental crosswalk cache (the original "persist a crosswalk for incremental joins" idea was superseded; see spec §2 + `decisions.md`).
...
**Status (2026-07-15): ✅ DONE.** Success metric met and verified live: all 17 CAs resolve to one identity (accounts + addresses linked), 85 duplicate companies collapsed, contacts/companies each resolve to a single ID, and both a match rate (tasks 1447/1447, calls 389/404 — the 6 gaps are AE Yianni + non-CA Tom Kennedy, correctly excluded; activity emails 3653/3689) and an unresolved-records list (`identity_unresolved`, 89 rows) are produced. PM check satisfied: match rates + the 17 linked identities + the review list were shown, three reps (Andrew/Kat/Yuvi) validated line-by-line, and a 5-agent production audit + verification pass was completed with all fixes applied and proven byte-identical. **Three items carried into Phase 3 (none are Phase 2 gaps — Phase 2 correctly PARKS unknowns):** (1) tryencord.com counting decision — `george@tryencord.com` (~98 emails) sits parked in `identity_unresolved` pending the PM+George call; (2) James Falconer "any other outreach tools?" completeness confirmation; (3) **wire `resolve.py` into `daily-run.yml`** — Phase 3's first task (until then it runs manually; re-run after editing `config/ca_teams.json`, a CA joining/leaving, or the tryencord decision).

Built as full-rebuild, reads raw tables only, run manually (not yet on the daily schedule). Outputs: `dim_ca`, `dim_ca_address`, `ca_amplemarket_user`, `company_crosswalk`, `contact_crosswalk`, `amplemarket_contact_map`, `identity_unresolved`. Two load-bearing guards found during validation (details + evidence in `decisions.md`): (1) never merge companies on domain alone — junk domains exist (google.com on unrelated companies); name must agree; (2) a CA can hold multiple AmpleMarket accounts — the rep↔ample link is a table, not a column. Open inputs that slot in later: tryencord.com counting decision; James F source-list confirmation. **Audited 2026-07-15** (5 parallel agents + a verification pass; fixes applied — see the production-audit entry in `decisions.md`): roster, attribution, and dedup foundations confirmed; the email dedup KEY was corrected in spec §3 before Phase 3 implements it. **Re-run `resolve.py` after:** editing `config/ca_teams.json`, a CA joining/leaving, or the tryencord decision — manual until wired into the cron; **wiring `resolve.py` into `daily-run.yml` (after formal PM sign-off) is an explicit early-Phase-3 checklist item.**
...
## Phase 3 — Unified activity model + flat view  ✅ BUILT 2026-07-16 (adversarially reviewed; PM check presented — see status below)
...
**Status (2026-07-16):** built as `model/rules.py` (all 11 rules below as pure, tested functions) + `model/build_activity.py` (deterministic full-rebuild of the `activity` fact table, single commit, with an end-of-run invariant check: every raw row referenced exactly once — a violation aborts the build) + `migrations/` (Git-managed schema: `activity` table, `dim_account`/`dim_contact`/`activity_flat` views) + `tests/` (rule tests on real validated fixtures; run on every push via `.github/workflows/tests.yml`). Both early-phase checklist items done: Supabase Pro upgraded before the build; `resolve.py` + migrations + model build wired into `daily-run.yml`. Success metric verified: flat view returns every activity with account/contact/channel attached; Andrew Bell 2026-07-14 and Yuvi 2026-07-15 side-by-sides reconcile exactly against the source tools; rebuild proven byte-identical across runs. Two body-key refinements found live and folded into spec §3 (client-banner strip; signature-suffix prefix match). Parked inputs unchanged (tryencord counting, Apollo counting, James-F tool list) — all built as identity/config toggles, not blockers.

**Work:** normalize staging into the activity fact table + account/contact/rep dimensions (per spec); attach each activity to its resolved account + contact; materialize the single flat analytics view; carry the HubSpot email `body` through from raw into the activity fact (captured from Phase 1 — see spec §7; AmpleMarket message text stays null until the v2 webhook). Connect Supabase↔GitHub for schema migrations here. **Early-phase checklist:** (a) the PM-decided **Supabase Pro upgrade must be in place before this build starts** (storage deadline — see the production-audit entry in `decisions.md`); (b) **wire `identity/resolve.py` into `.github/workflows/daily-run.yml`** (Phase 2 sign-off permitting) so the identity layer stops being a manual re-run.
...
3. **Same-email cross-tool duplicates** are N-way across varying sources; key (corrected 2026-07-15) = **sender + non-empty overlap of extracted external recipients + whitespace-normalized body prefix + ±180s window**; subject (tool- AND reply-prefix stripped) is tiebreak only — recipient-less subject+time collapse is forbidden except for invite-classified rows.
4. **Four** email sources incl. `CRM_UI`; unknown source ⇒ treat as manual, never drop.
5. **Invite/notification emails belong to the meeting channel** — classify by subject patterns, exclude from email counts; do NOT attempt row-level invite↔meeting joins.
6. **Calendar-invite rows** dedupe via the same corrected key; residual per-attendee fan-out (rare) collapses via invite-classified subject+time.
7. **AmpleMarket email task ↔ HubSpot send** reconciliation (tasks aren't sends).
8. **Calls: attempts vs conversations** counted separately; never collapse on `task_id` (usually null); conversation = same contact + tight window; real conversation only when `human = true`; ~25% of real conversations have no contact — surface, don't drop.
9. **Exclude internal-only-recipient** activity from outreach counts (over to ∪ cc ∪ bcc).
10. **Duplicate meeting objects** — dedupe meetings on (title, start_time, owner).
11. **Inbound gets the same dedup + noise filtering** — dedupe cross-tool reply copies; filter bounces/auto-replies/notification senders out of engagement metrics.
