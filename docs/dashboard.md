...
| `meeting_gong_verification` | NEW (migration 009, Ray ask #1): counted meetings with a COMPLETED Gong recording inside the booked slot sharing a contact — evidence the meeting happened. Splits the displayed "unknown" outcome into **held (Gong-verified)** + remaining unknown (the two always sum to scorecard's `meetings_unknown`). Labels only; no counted number changes; the two raw Gong tables stay closed to the reader. |
