...
2. **We never capture who an email was sent to, or meeting attendees.** Inspecting the actual stored `raw` jsonb confirms it only contains the specific properties requested (`hs_email_from_email`, subject, timestamp, source, owner) — not a recipient field for emails, nor an attendee/outcome field for meetings. This blocks Phase 3 rules already committed in this doc and the roadmap: internal-recipient exclusion and invite-fan-out collapsing both need the recipient list; attaching an outbound email to the prospect it went to needs it too; and meeting→rep attribution has no path at all without attendees, since `owner_id` is separately proven unreliable (see rep-identity entries above). Fix: add the missing properties to `ingest_hs_emails`/`ingest_hs_meetings`'s requested `props`, then re-run ingestion once for the days already captured (11–13 Jul) to backfill — cheap, since HubSpot still has the data; every day this isn't fixed is a day added to that backfill.
