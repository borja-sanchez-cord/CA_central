...
| **Meetings unknown** | Booked minus held minus canceled minus scheduled — i.e. no outcome was logged. | "Unknown" means *not logged*, **never** assume held. Any new outcome value HubSpot invents also lands here, not in held. On the dashboard this bucket is now **split** by the Gong evidence below. |
| **Meetings — held (Gong-verified)** | No outcome logged by the rep, but a **COMPLETED Gong recording** exists inside the booked slot (start −15 min .. end +30 min) **sharing a contact** with the meeting — evidence it happened (migration 009, Ray ask #1, 2026-07-23). | Display label carved out of *unknown*; the two always sum to the old unknown count — no counted number changes. A rep-entered outcome always wins. QUEUED Gong records prove nothing (future/never-happened calls sit QUEUED forever). No recording ≠ didn't happen (off-Gong meetings stay unknown). |
