...
- **Meeting attendees:** `python3 ingestion/ingest_meeting_contacts.py` — re-reads meeting→contact associations for ALL stored meetings (self-healing; runs daily in the cron). Feeds only migration 006's new-stakeholder meeting split; the verified model never reads it.
- **Deals:** `python3 ingestion/ingest_deals.py` — full re-read of ALL HubSpot deals + company links + the pipeline/stage schema (self-healing; runs daily in the cron). Feeds only migration 008's neglect labels (customer / open deal / recently lost / churned); the verified model never reads it.
